...
from typing import Callable, cast, Collection, IO, Iterable, MutableMapping, MutableSequence, Optional, Sequence, \
...
class LogCluster:
    ...
    def __init__(self, 
                 log_template_tokens: Sequence[str], 
                 cluster_id: int) -> None:
    ...
    def __str__(self) -> str:
        return f"ID={self.cluster_id} : size={self.size} : {self.get_template()}"
...
class DrainBase(ABC):
    def __init__(self,
                 ...
                 cluster_id_supplier: Optional[Callable[[], int]] = None) -> None:
        """
        ...
        self.cluster_id_supplier = cluster_id_supplier
    ...
    def __getstate__(self) -> MutableMapping[str, object]:
        state = self.__dict__.copy()
        state["cluster_id_supplier"] = None
        return state

    def __setstate__(self, state: MutableMapping[str, object]) -> None:
        self.__dict__.update(state)
        if "cluster_id_supplier" not in state:
            self.cluster_id_supplier = None
    ...
    def _next_cluster_id(self) -> int:
        if self.cluster_id_supplier is not None:
            cluster_id = self.cluster_id_supplier()
            if cluster_id > self.clusters_counter:
                self.clusters_counter = cluster_id
            return cluster_id
    ...
    def add_log_message(self, content: str) -> Tuple[LogCluster, str]:
        ...
        return self.add_log_message_tokens(content_tokens)

    def add_log_message_tokens(self, 
                                content_tokens: Sequence[str]) -> Tuple[LogCluster, str]:
        if self.profiler:
...
class Drain(DrainBase):
    ...
    def match(self, content_tokens: Sequence[str], full_search_strategy: str = "never") -> Optional[LogCluster]:
        """
